...
##############################
# source("ll2sim_3.r")
source("llmixdist.r")	# functions for generating/evaluating loglogistic and their mixtures
source("parsepara.r")	# parameter parser
...
#################################
fitll2 <- function(data, strpara='skip_mle', maxtime=1000, ylim=0.010, maintitle="", plot_pdf=TRUE, plot_haz=TRUE, do_mle=TRUE) {
	...
	} else {	# will only do empirical pdf/haz
	...
	if (plot_haz) {
		...
		plot(0,0, type='n', xlim=c(0,min(maxtime, 1000)), ylim=c(0,ylim*2.5), frame.plot=FALSE, xlab="Fixation Duration", ylab="Hazard Rate");
		title(main = maintitle)
		counter<-1
		for (i in names(stat)) {
